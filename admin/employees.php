...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_employee'])) {
        ...
        if ($username === '' || $password_plain === '' || $full_name === '' || $email_raw === '' || $latitude === '' || $longitude === '') {
            $message = "Please fill all fields, including location.";
            ...
            if ($check->fetchColumn() > 0) {
                ...
                try {
                    $stmt = $pdo->prepare("INSERT INTO users (username, email, password, full_name, role, latitude, longitude) VALUES (?, ?, ?, ?, 'employee', ?, ?)");
                    $stmt->execute([$username, $email, $password, $full_name, $latitude, $longitude]);
                    $message = "Employee added successfully with location!";
                } catch (Exception $e) {
                ...
                <form method="POST" class="employee-form">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="latitude" name="latitude" placeholder="e.g. 26.2389" required>
                        </div>

                        <div class="form-group">
                            <label for="longitude">Longitude:</label>
                            <input type="text" id="longitude" name="longitude" placeholder="e.g. 73.0243" required>
                        </div>
                        ...
                        <tbody>
                            <?php foreach ($employees as $employee): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($employee['latitude'] ?? 'N/A'); ?></td>
                                    <td><?php echo htmlspecialchars($employee['longitude'] ?? 'N/A'); ?></td>
                                    <td>
                                    ...
                                    <td class="actions">
                                        ...
                                        <form method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this employee?');">
                                            <input type="hidden" name="user_id" value="<?php echo $employee['id']; ?>">
                                            <button type="submit" name="delete_employee" class="btn-small btn-danger">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
